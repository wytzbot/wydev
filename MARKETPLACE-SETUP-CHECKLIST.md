...
- OAuth client: create a Web application client and add the exact production callback URL for the optional Google Drive integration: `/api/auth/google/callback`. Firebase Google Sign-In is configured separately under Firebase Authentication → Sign-in method → Google.

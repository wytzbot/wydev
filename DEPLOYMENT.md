...
## Flutterwave
Use sandbox first. Configure the production callback/redirect URL required by the selected v4 payment flow. Set the merchant encryption key as `FLW_ENCRYPTION_KEY`. WyteLab reads it server-side through `/billing/config`, so no `VITE_`-prefixed Flutterwave encryption variable is required in Vercel.
...
`.github/workflows/wydev-build.yml` notifies your deployed WyteLab instance at `https://wyte.name.ng/api/notifications/build-failed` when a build fails, so the failed-build push notification feature works out of the box.

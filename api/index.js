...
async function checkAIQuota(s){const day=new Date().toISOString().slice(0,10),used=await getUsage(s.id,day),plan=await entitlement(s),limit=plan==="pro"?Number(process.env.AI_PRO_DAILY_LIMIT||50):Number(process.env.AI_FREE_DAILY_LIMIT||3);if(used>=limit)throw Object.assign(new Error(`Daily AI diagnostic limit reached (${limit}).`),{status:429});await incrementUsage(s.id,day);}
function textFromResponse(data){if(data.output_text)return data.output_text;for(const item of data.output||[])for(const c of item.content||[])if(c.text)return c.text;return "";}

async function aiDiagnose(s,payload){
  await checkAIQuota(s);
  const files=Array.isArray(payload.relatedFiles)?payload.relatedFiles.slice(0,8):[];
  const context=JSON.stringify({error:payload.error||"",logs:String(payload.logs||"").slice(0,12000),file:payload.file||"",content:String(payload.content||"").slice(0,24000),relatedFiles:files.map(x=>({path:x.path,content:String(x.content||"").slice(0,10000)})),package:payload.package||""});
  const schema={type:"object",additionalProperties:false,properties:{
    title:{type:"string"},severity:{type:"string"},root_cause:{type:"string"},affected_files:{type:"array",items:{type:"string"}},
    affected_lines:{type:"array",items:{type:"string"}},evidence:{type:"array",items:{type:"string"}},
    likely_reason:{type:"string"},recommended_action:{type:"string"},confidence:{type:"number"}
  },required:["title","severity","root_cause","affected_files","affected_lines","evidence","likely_reason","recommended_action","confidence"]};
  const prompt=`You are WyDev Diagnostic Engine. Diagnose only. NEVER propose automatic code edits, patches, commits, file replacements, or autonomous actions. Identify the exact problem from the supplied minimum context. If evidence is insufficient say so. Return concise developer-readable JSON.\nCONTEXT:\n${context}`;
  const call=async model=>{const r=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{"Authorization":`Bearer ${process.env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model,input:prompt,store:false,max_output_tokens:900,text:{format:{type:"json_schema",name:"wydev_diagnostic",strict:true,schema}}})});const data=await r.json();if(!r.ok)throw new Error(data.error?.message||"AI provider failed");const t=textFromResponse(data);const out=JSON.parse(t);if(!out.root_cause||!Array.isArray(out.affected_files))throw new Error("AI response validation failed");return out};
  try{return await call(process.env.OPENAI_MODEL_PRIMARY||"gpt-5.4-nano")}catch(e){return await call(process.env.OPENAI_MODEL_FALLBACK||"gpt-5.4-mini")}
}
...
function amountFor(currency){if(currency==="NGN"){const n=Number(process.env.FLW_PRO_NGN);if(!n)throw new Error("FLW_PRO_NGN is required for NGN checkout");return n}return Number(process.env.FLW_PRO_USD||9.99);}
...
async function handler(req,res){
  try{
    ...
    if(p==="/github/repos"&&req.method==="GET")return json(res,200,await gh(s.token,"/user/repos?per_page=100&sort=updated"));
    ...
    if(cm&&req.method==="POST"){
      ...
      for(const c of changes){
        if(!c.path||c.status==="D"){entries.push({path:c.path,mode:"100644",type:"blob",sha:null});continue}
        const blob=await gh(s.token,`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/git/blobs`,{method:"POST",body:JSON.stringify({content:String(c.content??""),encoding:"utf-8"})});
    ...
    if(p==="/billing/config"&&req.method==="GET")return json(res,200,{usd:Number(process.env.FLW_PRO_USD||9.99),ngn:Number(process.env.FLW_PRO_NGN||0),environment:process.env.FLW_ENV||"sandbox"});

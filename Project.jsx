...
import { Copy, FilePlus, FolderPlus, ExternalLink, Upload, Trash2, Move, GitBranch, RefreshCw, ChevronDown, Loader2, Undo2, Download } from "lucide-react";
...
import { replaceAcrossFiles } from "../search";
import { copy } from "../utils";
import { loadState, saveState } from "../storage";
import { shouldSkipUpload, readFileText, readFileEntry, isBinaryData, base64ToBytes, guessMime, isZipFile, extractZipEntries, stripCommonRoot, safeRepoPath } from "../files";
import { promptDialog, confirmDialog } from "../dialog";
import { toastSuccess, toastError, toastInfo } from "../toast";
import Dropdown from "../components/Dropdown";

function BinaryFile({file,path}){
  const download=()=>{const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([base64ToBytes(file.base64)],{type:file.mime}));a.download=path.split("/").pop()||"download";a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)};
  const previewable=/^image\//.test(file.mime)||/\.(png|jpe?g|gif|webp|bmp|ico|svg)$/i.test(path);
  const src=previewable?`data:${previewable&&/^image\//.test(file.mime)?file.mime:guessMime(path)};base64,${file.base64}`:null;
  return <section className="panel binaryPreview">{src?<img src={src} alt={path} className="binaryImage"/>:<><h3>BINARY FILE</h3><p>{file.mime} · {file.size.toLocaleString()} bytes</p></>}<button onClick={download}><Download size={15}/> Download</button></section>;
}

export default function Project({ repo, onBack, onWorkingState, openPath }) {
  const key = `project:${repo.id}`;
  const checkpointsKey = `checkpoints:${repo.id}`;
  ...
  const [branch, setBranch] = useState(cached?.branch || repo.default_branch || "main"),
    ...
    // path -> ms timestamp it was last created/edited/uploaded/moved. Drives
    // the "now / 1 min ago / ..." labels in the file tree.
    [fileTimes, setFileTimes] = useState(cached?.fileTimes || {}),
    ...
    [checkpoints, setCheckpoints] = useState(() => loadState(checkpointsKey, [])),
    [checkpointsOpen, setCheckpointsOpen] = useState(false);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    saveState(key, { branch, files, base, baseSha, selected, fileTimes });
  }, [repo, branch, files, base, baseSha, selected, fileTimes, changes.length]);

  // Stamps one or more paths as "just touched now". Called alongside every
  // create/edit/upload/rename/move/duplicate so the file tree's relative-time
  // labels stay accurate. A path removed from `files` simply stops rendering,
  // so no separate cleanup is needed on delete.
  const touch = (paths) => {
    const ts = Date.now();
    const list = Array.isArray(paths) ? paths : [paths];
    setFileTimes((t) => {
      const next = { ...t };
      for (const p of list) next[p] = ts;
      return next;
    });
  };
  ...
  const load = async () => {
    try {
      ...
      setHistory([]);
      // Stamp every loaded path "now" so the tree has a starting time to show;
      // future edits/uploads move individual files forward from here via touch().
      const now = Date.now();
      setFileTimes(Object.fromEntries(Object.keys(map).map((p) => [p, now])));
  ...
  // Migrate older cached projects that predate relative timestamps. Every
  // existing file gets a sensible baseline instead of rendering without a
  // timestamp; files touched afterwards receive their real local timestamp.
  useEffect(() => {
    const paths = Object.keys(files);
    if (!paths.length) return;
    setFileTimes((current) => {
      const missing = paths.filter((p) => !current[p]);
      if (!missing.length) return current;
      const now = Date.now();
      const next = { ...current };
      missing.forEach((p) => { next[p] = now; });
      return next;
    });
  }, [files]);
  const edit = (v) => {
  ...
  const commit = async (msg) => {
    ...
    try {
      ...
      const cause = e?.message || "Unknown commit error";
      if (e?.status === 409) toastError(`${cause} Refresh the repository before retrying.`);
      else if (e?.status === 401) toastError(`${cause} Sign in with GitHub again.`);
      else if (e?.status === 413) toastError(`${cause} Reduce the number or size of files and commit again.`);
      else if (e?.status >= 500) toastError(`${cause} The server failed while creating the Git commit.`);
      else toastError(`Commit failed: ${cause}`);
    } finally {
      setBusy(false);
    }
  };
  const createFile = async () => {
    ...
    if (!result) return;
    let p;
    try { p = safeRepoPath(result.path); } catch (e) { toastError(e.message); return; }
    if (!p) return;
    if (p in files) { toastError(`A file already exists at ${p}`); return; }
  ...
  const createFolder = async () => {
    ...
    if (!result) return;
    let safe;
    try { safe = safeRepoPath(result.path.replace(/\\/g, "/").replace(/^\/+|\/+$/g, "")); } catch (e) { toastError(e.message); return; }
    if (!safe) return;
    if (Object.keys(files).some((p) => p === safe || p.startsWith(safe + "/"))) { toastError(`A folder or file already exists at ${safe}`); return; }
    ...
    touch(picked.length ? picked.map((p) => `${safe}/${p.split("/").pop()}`) : [`${safe}/.gitkeep`]);
  ...
  const deleteFolder = async () => {
    ...
    if (!result) return;
    const from = safeRepoPath(result.path.trim());
    if (!from) return;
  ...
  const rename = async () => {
    ...
    let from, to;
    try {
      from = safeRepoPath(result.from.trim());
      to = safeRepoPath(result.to.trim());
    } catch (e) { toastError(e.message); return; }
    if (!from || !to || from === to) return;
    if (Object.keys(files).some((p) => p === to || p.startsWith(to + "/"))) { toastError(`The destination folder ${to} already exists`); return; }
  ...
  const confirmRename = () => {
    ...
    touch(Object.keys(renamePreview.next).filter((p) => p.startsWith(renamePreview.to + "/") || p === renamePreview.to));
      ...
      for (const f of input.files) {
        if (isZipFile(f)) {
          ...
          for (const entry of stripCommonRoot(entries)) {
            ...
            // Same path already exists (e.g. a developer re-uploads a newer
            // export of a file/zip) -> this overwrite IS the update, no merge
            // step needed. We just count it separately so the toast is honest
            // about "replaced" vs "added", and touch() below refreshes its time.
            if (entry.path in next) updated++;
            else added++;
          ...
          const p = safeRepoPath(f.name);
          if (p in next) updated++;
          else added++;
          next[p] = /\.(png|jpe?g|gif|webp|ico|bmp|svgz|pdf|woff2?|ttf|otf|eot|mp3|mp4|mov|avi|webm|wav|exe|dll|so|dylib|class|jar|psd|ai|sqlite|db|zip|gz|tar|7z|rar)$/i.test(p)
            ? await readFileEntry(f)
            : await readFileText(f);
          touched.push(p);
        }
      }
      applyFiles(next, `Uploaded ${added + updated} file${added + updated === 1 ? "" : "s"}`);
      touch(touched);
      const parts = [];
      if (added) parts.push(`${added} added`);
      if (updated) parts.push(`${updated} updated to the new version`);
      if (skippedBinary) parts.push(`${skippedBinary} binary file${skippedBinary === 1 ? "" : "s"} imported`);
      toastSuccess(parts.length ? `${parts.join(" · ")} · staged locally` : "Nothing new to stage");
    } catch (err) {
  ...
  const exportZip = async () => {
    ...
    try {
      ...
      const zip = new JSZip();
      Object.entries(files).forEach(([path, content]) => zip.file(path, isBinaryData(content) ? base64ToBytes(content.base64) : content));
  ...
  // ---------------------------------------------------------------------
  // Pro features below are all pure client-side computation over state
  // already held in memory (files/fileTimes) or plain localStorage — no new
  // API calls, no server storage, so they add value without adding cost.
  // ---------------------------------------------------------------------

  // Named local checkpoints: a lightweight, labeled save point beyond what
  // Undo covers (Undo is linear and lost on refresh; checkpoints are named,
  // browsable, and persist in localStorage). Free users see the feature but
  // it's gated like Export ZIP / Pull Requests elsewhere in this file.
  const saveCheckpoint = async () => {
    if (!isPro) {
      toastError("Named checkpoints are a WyDev Pro feature. Upgrade to unlock them.");
      return;
    }
    const result = await promptDialog({
      title: "Save checkpoint",
      confirmLabel: "Save",
      fields: [{ key: "label", label: "Checkpoint name", placeholder: "Before refactor" }],
    });
    if (!result) return;
    const label = result.label.trim();
    if (!label) return;
    const next = [{ id: `${Date.now()}`, label, ts: Date.now(), files, fileTimes }, ...checkpoints].slice(0, 20);
    setCheckpoints(next);
    saveState(checkpointsKey, next);
    toastSuccess(`Checkpoint "${label}" saved`);
  };
  const restoreCheckpoint = async (cp) => {
    const ok = await confirmDialog({
      title: "Restore checkpoint",
      message: `Restore "${cp.label}"? Your current working state will be replaced (this itself becomes an Undo step).`,
      confirmLabel: "Restore",
      danger: true,
    });
    if (!ok) return;
    applyFiles(cp.files, `Restored checkpoint "${cp.label}"`);
    setFileTimes(cp.fileTimes || {});
    setCheckpointsOpen(false);
    toastSuccess(`Restored "${cp.label}"`);
  };
  const deleteCheckpoint = (id) => {
    const next = checkpoints.filter((c) => c.id !== id);
    setCheckpoints(next);
    saveState(checkpointsKey, next);
  };

  // Bulk find & replace across the whole working tree in one pass — handy for
  // renaming a variable or import path project-wide without opening every file.
  const runReplace = async () => {
    if (!isPro) {
      toastError("Multi-file Find & Replace is a WyDev Pro feature. Upgrade to unlock it.");
      return;
    }
    const result = await promptDialog({
      ...
      fields: [
        { key: "find", label: "Find", placeholder: "oldFunctionName" },
        { key: "replaceWith", label: "Replace with", placeholder: "newFunctionName", required: false },
        { key: "regex", label: "Treat \"Find\" as a regular expression", type: "checkbox" },
      ],
    });
    if (!result || !result.find) return;
    try {
      const { next, changedFiles, occurrences } = replaceAcrossFiles(files, result.find, result.replaceWith || "", !!result.regex);
      if (!changedFiles) {
        toastInfo("No matches found");
        return;
      }
      applyFiles(next, `Replaced "${result.find}" in ${changedFiles} file${changedFiles === 1 ? "" : "s"}`);
      touch(Object.keys(next).filter((p) => next[p] !== files[p]));
      toastSuccess(`${occurrences} occurrence${occurrences === 1 ? "" : "s"} replaced across ${changedFiles} file${changedFiles === 1 ? "" : "s"}`);
    } catch (e) {
      toastError(e.message);
    }
  };
  ...
  return (
    <div className="project">
      <header className="projectHeader">
        ...
        <Dropdown
          value={branch}
          onChange={(v) => switchBranch({ target: { value: v } })}
...
/>
      ...
      <div className="projectTools">
        ...
        <button onClick={saveCheckpoint}>
          Save checkpoint{plan !== "pro" ? " (Pro)" : ""}
        </button>
        <button onClick={() => setCheckpointsOpen((o) => !o)} disabled={!checkpoints.length}>
          Checkpoints{checkpoints.length ? ` (${checkpoints.length})` : ""}
        </button>
        <button onClick={runReplace}>
          Find &amp; Replace{plan !== "pro" ? " (Pro)" : ""}
        </button>
      </div>
      {prOpen && (
      ...
      <div className={`projectBody${selected && selected in files ? " hasSelection" : ""}`}>
        <aside>
          <FileExplorer files={files} fileTimes={fileTimes} onOpen={setSelected} isPro={isPro} />
        </aside>
        <main>
      ...
      {checkpointsOpen && (
        <div className="sheet">
          ...
          {checkpoints.length ? (
            checkpoints.map((cp) => (
              <div className="ref" key={cp.id}>
                <b>{cp.label}</b> — {Object.keys(cp.files).length} files, saved {new Date(cp.ts).toLocaleString()}
                <span className="sheetActions">
                  <button className="primary" onClick={() => restoreCheckpoint(cp)}>
                    Restore
                  </button>
            ...
            <p className="muted">No checkpoints saved yet.</p>
          )}

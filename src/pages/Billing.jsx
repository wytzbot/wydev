...
export default function Billing(){
 ...
 if(status.plan==="pro")return <div className="page"><header><div><span className="eyebrow">BILLING</span><h1>WyDev Pro</h1></div></header><section className="panel"><h3>PLAN</h3><p className="proActiveBadge"><b>YOU ARE ON THE PRO PLAN</b></p><p className="muted">{status.expiresAt?`Active until ${new Date(status.expiresAt).toLocaleDateString()}.`:""} Payment confirmations are verified server-side before Pro access is granted. For your security, no payment or card details are ever displayed here.</p></section>{success&&<SuccessModal onContinue={()=>location.reload()}/>}</div>;

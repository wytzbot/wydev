...
export default function Billing(){
 const [status,setStatus]=useState({plan:"free"}),[cfg,setCfg]=useState({usd:1,ngn:1000,encryptionKey:""}),[currency,setCurrency]=useState("NGN"),[busy,setBusy]=useState(false),[err,setErr]=useState(""),[card,setCard]=useState({number:"",expiry:"",cvv:""}),[auth,setAuth]=useState(null),[authValue,setAuthValue]=useState(""),[authFields,setAuthFields]=useState({}),[authMessage,setAuthMessage]=useState(""),[success,setSuccess]=useState(false);
 ...
 if(status.plan==="pro")return <div className="page"><header><div><span className="eyebrow">BILLING</span><h1>WyDev Pro</h1></div></header><section className="panel"><h3>PLAN</h3><p className="proActiveBadge"><b>YOU ARE ON THE PRO PLAN</b></p><p className="muted">{status.expiresAt?`Next renewal: ${new Date(status.expiresAt).toLocaleDateString()}.`:""} Monthly recurring subscription. Payment confirmations are verified server-side before Pro access is granted. For your security, no payment or card details are ever displayed here.</p>{success&&<p className="successText">Payment successful. Pro is active.</p>}</section></div>;
 return <div className="page"><header><div><span className="eyebrow">BILLING</span><h1>WyDev Pro</h1></div></header><section className="panel proOffer"><h3>PRO INCLUDES</h3><ul className="featureList"><li><b>More GitHub repositories</b><span>Remove the free repository limit and keep more repositories in your WyDev workspace.</span></li><li><b>5 AI diagnoses/day</b><span>Get a higher daily AI diagnosis allowance; answers stay concise.</span></li><li><b>Revert repository</b><span>Restore a branch to an earlier commit without rewriting its Git history.</span></li><li><b>Rerun failed workflows</b><span>Retry failed GitHub Actions jobs directly from WyDev, including optional debug logging.</span></li></ul><p className="muted">Your Pro benefits are tied to your WyDev account and remain available while your subscription is active.</p></section><section className="panel"><h3>PLAN</h3><div className="price">{currency==="NGN"?(cfg.ngn?`₦${cfg.ngn.toLocaleString()}`:"NGN price not configured"):`$${(cfg.usd??1).toFixed(2)}`}/month · recurring</div><Select value={currency} onChange={setCurrency} options={[{value:"NGN",label:"NGN — ₦"},{value:"USD",label:"USD — $"}]} />{!auth?<><input className="searchInput" inputMode="numeric" autoComplete="cc-number" placeholder="Card number" value={card.number} onChange={e=>setCard({...card,number:e.target.value.replace(/\D/g,"")})}/><input className="searchInput" inputMode="numeric" autoComplete="cc-exp" placeholder="MM/YY" value={card.expiry} onChange={e=>setCard({...card,expiry:e.target.value.replace(/[^0-9/]/g,"")})}/><input className="searchInput" inputMode="numeric" autoComplete="cc-csc" placeholder="CVV" value={card.cvv} onChange={e=>setCard({...card,cvv:e.target.value.replace(/\D/g,"")})}/><button className="primary" disabled={busy} onClick={pay}>{busy?"Processing…":"Pay securely with Flutterwave"}</button></>:<div className="authBox"><p><b>{authLabel}</b></p>{authMessage&&<p className="muted">{authMessage}</p>}{auth.kind==="fields"?auth.fields.map(f=>{const key=String(f).split(".").pop();return <input key={key} className="searchInput" placeholder={key.replace(/_/g," ")} value={authFields[key]||""} onChange={e=>setAuthFields({...authFields,[key]:e.target.value})}/> }):<input className="searchInput" type={auth.kind==="otp"?"text":"password"} inputMode={auth.kind==="pin"||auth.kind==="otp"?"numeric":"text"} autoComplete={auth.kind==="otp"?"one-time-code":"off"} placeholder={authPlaceholder} value={authValue} onChange={e=>setAuthValue(e.target.value)}/>}<button className="primary" disabled={busy} onClick={authorize}>{busy?"Authorizing…":"Continue payment"}</button><button className="ghost" type="button" disabled={busy} onClick={()=>{setAuth(null);setAuthValue("");setAuthMessage("")}}>Cancel</button></div>}<p className="muted">Card fields are encrypted in your browser with AES-256 before WyDev sends payment data to Flutterwave. Your card data is sent only to the payment API and is not stored by WyDev. WyDev does not store your card PIN, password or OTP.</p>{err&&<p className="error">{err}</p>}</section></div>
}

...
// text or a {__wydevBinary} payload, matching the shape extractZipEntries
// already produces for zipped binaries. Without this, any image/font/pdf/etc.
// picked directly (not inside a zip) was read with file.text(), which decodes
// raw bytes as UTF-8 and permanently corrupts the content before it's ever
// staged or committed.
export async function readUploadedFile(file,path){
  if(BINARY_EXT.has(extOf(path))){
    const buf=await file.arrayBuffer();
    let binary="";
    const bytes=new Uint8Array(buf);
    for(let i=0;i<bytes.length;i++) binary+=String.fromCharCode(bytes[i]);
    return {__wydevBinary:true,base64:btoa(binary),mime:file.type||mimeForUploadPath(path),size:file.size||0};
  }
  return file.text();
}

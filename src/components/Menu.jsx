...
export default function Menu({page,setPage,onLogout,onSearch}){
 ...
 return <aside className="menu"><div className="brand">WYTE</div><input className="globalSearch" value={q} placeholder="Search actions, files, repos…" onChange={e=>{setQ(e.target.value);if(e.target.value) {go("search");onSearch?.(e.target.value)}}}/>{items.map(([id,label,I])=><button className={page===id?"active":""} key={id} onClick={()=>go(id)}><I size={18}/>{label}</button>)}<button className="logout" onClick={onLogout}><LogOut size={18}/>Sign out</button></aside>
}

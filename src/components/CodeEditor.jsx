...
export default function CodeEditor({path,value,onChange,fontSize=16,wordWrap=true,onViewReady}){const [text,setText]=useState(value||"");useEffect(()=>setText(value||""),[path]);return <div className="editor"><CodeMirror value={text} height="calc(100vh - 128px)" theme={oneDark} extensions={[lang(languageFor(path))]} onChange={v=>{setText(v);onChange(v)}} basicSetup={{lineNumbers:true,foldGutter:true,wordWrap}} onCreateEditor={view=>onViewReady?.(view)} style={{fontSize}}/></div>}

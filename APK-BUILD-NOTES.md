...
- **Commit endpoint (`api/index.js`) — path normalization was validated but discarded.** `safePath()` strips leading slashes and converts backslashes to forward slashes, but the loop that validated `changes[].path` before building the Git tree threw away the normalized result and used the raw path anyway. A path with a leading `/` or a Windows-style `\` would pass validation but still get sent to GitHub's Trees API un-normalized, producing a literally-named file instead of the intended nested path. Fixed to write the normalized path back: `c.path = safePath(c.path)`.
- **Android shell (`MainActivity.java`) — file/folder upload picker was non-functional.** `APK-BUILD-NOTES.md` already documented this gap: a bare `WebView` never shows a file picker for `<input type="file">` unless the hosting `Activity` implements `WebChromeClient.onShowFileChooser`. Added a `WebChromeClient` with `onShowFileChooser` plus the matching `onActivityResult` handler to deliver the picked file(s) back to the page. This only affects the custom `.wydev/android-shell` build; it doesn't apply if you're building through Median instead.

## If "Repository creation failed" persists

The create-repo route already returns specific reasons (invalid name characters, GitHub rate limiting, missing OAuth scopes, name already taken, etc.) and the client already shows that exact message in the toast — there's no bug in this pass that would make it fail universally with no useful detail. Things worth checking:
